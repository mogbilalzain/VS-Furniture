...
export default function ProductDetails() {
  ...
  const loadProductFiles = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000/api'}/products/${params.id}/files`, {
        method: 'GET',
        headers: {
      ...
      if (response.ok) {
        ...
        if (data.success) {
          ...
          console.log('✅ Product files loaded:', data.data.length, 'files');
        }
      }
    } catch (err) {
  ...
  const handleDownloadFile = async (fileId, fileName) => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000/api'}/products/${params.id}/files/${fileId}/download`, {
        method: 'GET',
        headers: {
    ...
    <>
      ...
      <main className="bg-gray-000">
            ...
            <div className="grid lg:grid-cols-2 gap-16 items-center">
              ...
              <div className="order-2 lg:order-1">
                ...
                <p className="text-gray-600 mb-8 text-lg leading-relaxed">
                  {product.short_description || product.description || 'Available in various configurations to meet your needs'}
                </p>
          ...
          <div className="container mx-auto px-4">
            <div className="max-w-4xl">
              ...
              <div className="text-lg leading-relaxed mb-6">
                {product.description || product.short_description || 'Product description not available.'}
              </div>
        ...
        <div className="bg-gray-000 py-16">
          <div className="container mx-auto px-4">
            <ProductFiles productId={params.id} />
          </div>
        </div>
      </main>
