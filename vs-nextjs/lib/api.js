...
class ApiClient {
  ...
  async request(endpoint, options = {}) {
    ...
    // Debug public endpoint detection for all admin requests
    ...
    if (token && needsAuth) {
      ...
      } else if (isPublicProductCategory) {
        console.log(`🔓 No Authorization header for public ${endpoint} (public endpoint)`);
      } else if (endpoint.includes('admin/') || (endpoint.includes('categories') && !endpoint.includes('auth/')) || (endpoint.includes('products') && !endpoint.includes('auth/'))) {
        console.log(`❌ No Authorization header for ${endpoint}`);
        console.log(`❌ Reason - Token exists: ${!!token}, Is public: ${isPublicEndpoint}, Is public product/category: ${isPublicProductCategory}`);
      }
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          ...
          // 3. Public endpoints (they shouldn't have token anyway)
          if (typeof window !== 'undefined' && 
            ...
            console.log('🔐 Reason:', 
              ...
              isPublicEndpoint ? 'Public endpoint' :
              isPublicProductCategory ? 'Public product/category endpoint' :
              'Unknown'
            );
          }
      ...
      if (error.name === 'TypeError' && error.message === 'Failed to fetch') {
        const networkError = new Error('🌐 Cannot connect to server. Please make sure the Laravel backend is running on http://localhost:8000');
...
export const productsAPI = {
  ...
  create: (productData) => {
    ...
    if (productData instanceof FormData) {
      return apiClient.postFormData('/products', productData);
    }
