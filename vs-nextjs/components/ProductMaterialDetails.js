  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log('🔍 ProductMaterialDetails: Fetching data for product ID:', productId);

        // Fetch categories and product materials in parallel
        const [categoriesResponse, productMaterialsResponse] = await Promise.all([
          materialsAPI.getCategories(),
          productId ? materialsAPI.getProductMaterials(productId) : Promise.resolve({ data: { materials_by_category: [] } })
        ]);
        ...
        console.error('❌ ProductMaterialDetails: Error fetching data:', err);
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-6xl mx-auto">
            ...
            <p className="text-gray-500">
              No material categories available at the moment.
            </p>
          </div>
        </div>
      </section>
    ...
    <section className="py-16 md:py-24 bg-gray-000">
      <div className="container mx-auto px-4">
        <div className="max-w-6xl mx-auto">
          ...
          {/* Material Groups */}
          {selectedCategory && (
            <div role="tabpanel" aria-labelledby={`tab-${selectedCategory.id}`}>
              {selectedCategory.groups && selectedCategory.groups.length > 0 ? (
                ...
                <div className="text-center py-8">
                  <p className="text-gray-500">
                    No material groups available in this category.
                  </p>
                </div>
              )}
            </div>
